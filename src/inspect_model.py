...
def forward_hooks(modules: dict) -> tuple[dict, list]:
    """Навесить forward-hooks и вернуть хранилище вместе с handle каждого хука."""
...
def activation_norms(tokenizer, model, params: dict) -> dict:
    ...
    try:
        ...
        for handle in handles:
            handle.remove()
    hooks_after = sum(len(module._forward_hooks) for module in model.modules())
...
def device_allocated_bytes(device: torch.device) -> int:
    """Сколько памяти занято на выбранном устройстве прямо сейчас."""
    if device.type == "cuda":
...
class PeakMemory:
    """Пиковая память процесса или ускорителя за измеряемый участок."""
    ...
    def __enter__(self) -> "PeakMemory":
        if self.device.type == "cuda":
            ...
            self._thread = threading.Thread(target=self._sample_mps, daemon=True)
            self._thread.start()
        return self
...
def memory_profile(params: dict) -> list[dict]:
    ...
    for mode in MODES:
        ...
        for _ in range(repeats):
            ...
            if not lines:
                raise RuntimeError(f"режим {mode} не вернул результат")
...
def main() -> None:
    ...
    if args.probe:
        # Служебный JSON перенаправляется в файл из Git Bash. ASCII-экранирование
        # исключает зависимость от активной кодовой страницы Windows.
